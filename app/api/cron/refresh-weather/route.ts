...
async function refreshAll(ctx: CronContext, request: Request) {
  ...
  let resortQuery = supabase.from("resorts").select(`${RESORT_COLUMNS}, currently_open`).eq("active", true);
  ...
  for (const o of okOutcomes) {
    ...
    toClassify.push({ id: r.id, code: classifyToday(window.slice(-8))?.code ?? null });
  }
  const BATCH = 20;
