...
export function cityOptions(): Array<{ code: string; label: string }> {
  return LAUNCH_CITIES.map((c) => ({ code: c.code, label: c.name }));
}
...
export function originLabel(origin: GoOrigin): string {
  return origin.kind === "city" ? origin.name : "your location";
}
...
/**
 * Query string for the map planner's `from=` origin. Cities in the map's
 * own list (ORIGINS) go by code; other launch cities and geo origins go
 * as coordinates plus a label so the planner shows "Chicago", not "Your
 * location". Mirrors app/trip-templates/[slug]/page.tsx.
 */
export function plannerOriginParams(origin: GoOrigin): string {
  if (origin.kind === "city" && origin.driveCache) return `from=${origin.code}`;
  const label = origin.kind === "city" ? origin.name : "Your location";
  return (
